#!/usr/bin/env th
--[[
A Pattern Language - Sequence 1: Regions instead of countries
...
local SEQUENCE_INFO = {
  id = 1,
  heading = "Regions instead of countries",
  description = "Sequence 1 focuses on regions instead of countries",
  category = "Towns",
  pattern_ids = {1},
  emergent_phenomena = "Autonomous regions that can govern themselves and maintain their unique cultural identity"
}
...
local TextEncoder, parent = torch.class('nn.TextEncoder_Seq1', 'nn.Module')
...
local PatternEncoder, parent = torch.class('nn.PatternEncoder_Seq1', 'nn.Module')
...
function PatternEncoder:__init(num_patterns, pattern_dim, text_hidden, output_dim)
  ...
  self.problem_encoder = nn.TextEncoder_Seq1(256, 64, text_hidden)
  self.solution_encoder = nn.TextEncoder_Seq1(256, 64, text_hidden)
...
local PatternGNN, parent = torch.class('nn.PatternGNN_Seq1', 'nn.Module')
...
local SequenceModel, parent = torch.class('nn.SequenceModel_Seq1', 'nn.Module')
...
function SequenceModel:__init(num_patterns, num_categories, embed_dim)
  ...
  self.encoder = nn.PatternEncoder_Seq1(num_patterns, 128, 128, embed_dim)
  self.gnn = nn.PatternGNN_Seq1(embed_dim, embed_dim, 2)
...
function demo()
  print(string.rep("=", 60))
  print(string.format("Sequence %d: %s", 1, "Regions instead of countries"))
  print(string.rep("=", 60))
  print(string.format("Category: %s", "Towns"))
  print(string.format("Patterns: %s", "1"))
  print(string.format("\nEmergent Phenomenon:\n%s", "Autonomous regions that can govern themselves and maintain their unique cultural identity"))
  print(string.rep("=", 60))
  
  local model = nn.SequenceModel_Seq1(253, 3, 256)
...
if arg and arg[0]:match("seq01_nn.lua$") then
  demo()
end

return {
  SequenceModel = nn.SequenceModel_Seq1,
  PATTERNS = PATTERNS,
